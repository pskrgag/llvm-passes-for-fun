...
def main() -> None:
    ...
    with subtest("run in bounds"):
        run_project_executable("memory-safety/build/use-after-free.out", args=[])

    with subtest("run out of bounds"):
        result = run_project_executable("memory-safety/build/use-after-free.out", args=[""], stderr=subprocess.PIPE,
                                        check=False)
